% this script tunes LLL parameters on clean MNIST data
rng(32);
warning('off')
%% LLL
N = 70000;
...
sprintf('Setup: clean MNIST dataset size=%d', N)
...
for s = bandwidth
    for kW = knn 
        ...
        path = sprintf('/home/grad/chaos/Desktop/s%d/kW%d', s, kW);
        %mkdir(path)
        cd(path)
        %save('LLL_affinity.mat', 'W');
        load('LLL_affinity.mat')
        for kZ = KZ
            ...
            for nl = normalization 
                ...
                for index = 1:3 
                    ...
                    path = sprintf('/home/grad/chaos/Desktop/s%d/kW%d/kZ%d/nl%d/beta%.1f', s, kW,kZ,nl,beta);
                    mkdir(path)
                    cd(path)
...
fprintf('Finished grid search on clean data.')
for i = 1:3
